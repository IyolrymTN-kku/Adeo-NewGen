...
const FOOTER_NAV = {
  solutions: [
    { href: "/solutions", namespace: "nav", key: "solutions" },
    { href: "/cloud", namespace: "nav", key: "cloud" },
  ],
  company: [
    { href: "/about", namespace: "footer", key: "about" },
    { href: "/contact", namespace: "footer", key: "contact" },
  ],
  legal: [
    { href: "/privacy", namespace: "footer", key: "privacy" },
    { href: "/terms", namespace: "footer", key: "terms" },
  ],
} as const;
...
export async function Footer({ companyName }: FooterProps = {}) {
  const tFooter = await getTranslations("footer");
  const tNav = await getTranslations("nav");
  const settings = await prisma.companySettings.findUnique({ where: { id: 1 } });
  const displayCompanyName = companyName ?? settings?.companyName ?? "ADEO Solution";
      ...
      <Container>
        <div className="grid gap-12 py-16 lg:grid-cols-12">
          ...
          <div className="lg:col-span-5">
            <Logo invert companyName={displayCompanyName} logoUrl={settings?.logoUrl} />
                  ...
                  {links.map((link) => (
                    <li key={link.href}>
                      <Link href={link.href} className="text-sm text-slate-400 transition hover:text-[#3385ff]">
                        {link.namespace === "nav" ? tNav(link.key) : tFooter(link.key)}
                      </Link>
                    </li>
                  ))}
                </ul>
        ...
        <div className="flex flex-col items-start justify-between gap-3 border-t border-white/10 py-6 text-xs text-slate-500 sm:flex-row sm:items-center">
          <p>© {new Date().getFullYear()} {displayCompanyName}. {tFooter("rights")}</p>

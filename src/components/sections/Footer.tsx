...
const footerAccent  = "var(--site-footer-accent, var(--admin-accent, #3385ff))";
    ...
    <footer className="mt-auto" style={{ backgroundColor: footerBg, color: footerFg }}>
      ...
      <Container>
          ...
          <div className="lg:col-span-5">
            ...
            <div className="mt-6 grid grid-cols-2 gap-x-8 gap-y-2">
              ...
              <div className="space-y-2 text-sm" style={{ color: footerMuted }}>
                {settings?.taxId && (
                  <p>
                    <span className="font-semibold" style={{ color: footerHeading }}>{t("taxId")}</span>{" "}
                    {settings.taxId}
                  </p>
                )}
                {settings?.email && (
                ...
                {settings?.phone && (
                  <p>
                    <span className="font-semibold" style={{ color: footerHeading }}>{t("phone")}</span>{" "}
                    {settings.phone}
                  </p>
                )}
                {settings?.address && (
                  <p>
                    <span className="font-semibold" style={{ color: footerHeading }}>{t("address")}</span>{" "}
                    {settings.address}
                  </p>
                )}
                {settings?.website && (
                  <p>
                    <span className="font-semibold" style={{ color: footerHeading }}>{t("website")}</span>{" "}
                    <a
                  ...
                  {settings.linkedin && (
                    ...
                    >
                      ...
                      <span className="truncate">
                        {settings.linkedin.replace(/.*linkedin\.com\/(company\/|in\/)?/, "").replace(/\/$/, "")}
                      </span>
                    </a>
                  )}
                  {settings.instagram && (
        ...
        <div
          ...
          style={{ borderColor: footerBorder, color: footerSubtle }}
        >
          <p>
            © {new Date().getFullYear()} {settings?.companyName ?? companyName}.{" "}
            {t("rights")}
          </p>
          <p>{t("location")}</p>
        </div>
